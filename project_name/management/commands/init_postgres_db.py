...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        #  Make sure we're running with local settings
        if 'local' not in os.environ.get('DJANGO_SETTINGS_MODULE'):
            raise CommandError("This command needs to be run with local settings")

        if not options['force']:
            confirm = raw_input('This will drop your current postgresql databases.  \
                                Are you sure? (n): ')
        ...
        try:
            ...
            logger.exception('Error encountered while initializing postgresql \
                             databases:')
